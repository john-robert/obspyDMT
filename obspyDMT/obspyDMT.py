...
def command_parse():
    ...
    helpmsg = "state phases to take into consideration when using option " \
              "\"--cut_time_phase\"." \
            ...
            if os.path.exists(os.path.join(target_path, 'info',
                ...
                for bulk_sta in Sta_fdsn:
                    if input['cut_time_phase']:
                        t_start, t_end = calculate_time_phase(input, event, bulk_sta)
                    else:
                        t_start = event['t1']
                        t_end = event['t2']
...
def FDSN_download_core(i, j, dic, type, len_events, events,
    ...
    try:
        ...
        if input['cut_time_phase']:
            t_start, t_end = calculate_time_phase(input, events[i], Sta_req[j])
            # abort condition if no given phases match phases 
            # calculated by 'taup.getTravelTimes()' (see there)
            if t_start == 0 and t_end == 0:
                ee = 'No %s phase(s) expected for Rvent %s --- skip: %s.%s.%s.%s' \
                      % (input['phases'], 
                         i+1, 
                         Sta_req[j][0], 
                         Sta_req[j][1], 
...
def calculate_time_phase(input, event, sta):
    """
    ...
    phase_list = input['phases']
    flag = False
    for ph in phase_list:
        for i in range(len(tt)):
            #print tt[i]['phase_name']		# testing purposes
    ...
    # In case no given phases match phases 
    # calculated by 'taup.getTravelTimes()',
    # t_start & t_end are set to 0 in order 
    # to create possibility for an abort condition
    # for function calling 'calculate_time_phase(..)'    
    try:
...
def FDSN_ARC_IC(input, clients):
    ...
    for i in range(len(events)):
        ...
        if len(ls_saved_stas) != 0:
            ...
            print "\nThere is no station in the directory to correct!"
